namespace :dataset do
  ...
  task create: :environment do
    replace_dataset_metadata("http://linked-development.org/data/eldis", "http://linked-development.org/graph/eldis", 'Eldis Data', 'This dataset is drawn from ELDIS, an online information service hosted by the Institute for Development Studies. It provides up-to-date and diverse research on international development issues.', <<-MARKDOWN)
The data in this dataset is updated every night via the [IDS Knowledge Services API](http://api.ids.ac.uk/).
    MARKDOWN
    replace_dataset_metadata("http://linked-development.org/data/r4d", "http://linked-development.org/graph/r4d", 'R4D Data', 'This dataset is from Research for Development (R4D), it includes details of research publications and projects funded by the UK Department for International Development.', <<-MARKDOWN)
